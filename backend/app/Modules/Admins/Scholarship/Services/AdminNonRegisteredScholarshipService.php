...
class AdminNonRegisteredScholarshipService
{
	...
	protected function model(): Builder
	{
		return Application::with([
			...
			'student'=> fn($query) => $query->with(['roles']),
		])
			...
			->where(function($query){
				$query->whereHas('institute', fn($query) => $query->with(['auth' => fn($q) => $q->with('address')])->where(function($query){
					$query->doesntHave('auth')
					->orWhereHas('auth', fn($query) => $query->doesntHave('profile')->orWhereHas('profile', fn($query) => $query->where('is_blocked', true)))
					->orWhere('is_active', false);
				}))
				->orWhereHas('industry', fn($query) => $query->where(function($query){
					$query->doesntHave('auth')
					->orWhereHas('auth', fn($query) => $query->where('is_blocked', true))
					->orWhere('is_active', false);
				}));
			})
			->with([
