...
use App\Modules\InstituteManagement\Authentication\Requests\ResetPasswordPostRequest;
        ...
        if($data){
            if($data->created_at->diffInMinutes(now()) < (int)config('auth.reset_password_link_timeout')){
                ...
                if($user->otp == $request->otp){
                    $this->authService->updateInstituteAuth($user, ['otp' => rand (1111, 9999), 'password' => $request->password]);
                    $this->authService->deletePasswordResetLink($token);
