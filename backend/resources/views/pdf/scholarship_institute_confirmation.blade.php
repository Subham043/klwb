            ...
            <div class="row">
                <div class="col m10 push-m1">
                    <div class="col s12 center">
                        <p>ಭಾಗ-3</p>
                        <p>ಶಿಕ್ಷಣ ಸಂಸ್ಥೆಯ ಮುಖ್ಯಸ್ಥರ ಪ್ರಮಾಣ ಪತ್ರ</p>
                        <p>(ಶಿಕ್ಷಣ ಸಂಸ್ಥೆಯವರು ಭರ್ತಿ ಮಾಡಿ ತಪ್ಪದೇ ದೃಡೀಕರಿಸವುದು)</p>
                    </div>
                    <div class="col s12">
                        <ol>
                            <li>
                                <div>
                                    <p>ಕುಮಾರ/ಕುಮಾರಿ <span class="dotted-line"> {{ $application->basic_detail->name }}
                                        </span> ಇವರು ಕಳೆಧ ಸಾಲಿನಲ್ಲಿ
                                        ನಡೆದ <span class="dotted-line"> {{ $application->mark->prv_class }} </span>
                                        ತರಗತಿಯ ಪರೀಕ್ಷೆಯಲ್ಲಿ ಶೇಕಡಾ <span class="dotted-line">
                                            {{ $application->mark->prv_marks }}% </span> ಅಂಕಗಳನ್ನು
                                        ಪಡೆದ್ದು ತೇರ್ಗಡೆಯಾಗಿರುತಾರೆ.</p>
                                </div>
                            </li>
                            <li>
                                <div>
                                    <p>{{ date('Y', strtotime('-1 years')) . '-' . date('Y') }}ನೇ ಶೈಕ್ಷಣಿಕ ಸಾಲಿನಲ್ಲಿ
                                        <span class="dotted-line"> {{ $application->mark->graduation->name ?? '' }}
                                            {{ $application->mark->course->name ?? '' }}
                                            {{ $application->mark->class->name ?? '' }} </span> ತರಗತಿಯಲ್ಲಿ ವ್ಯಾಸಂಗ
                                        ಮಾಡುತ್ತಿದ್ದಾರೆಂದು ದೃಡೀಕರಿಸಲಾಗಿದೆ.</p>
                                </div>
                            </li>
                        </ol>
                    </div>
                    <div class="col s12 m5 foo-address left">
                        ...
                        <p>ದಿನಾಂಕ: <span class="dashed-line">
                                {{ $application->school_approve ? $application->school_approve->format('d M, Y') : '' }}
                            </span></p>
                        <p>ದೂರವಾಣಿ ಸಂಖ್ಯೆ (STD CODE ಸಹಿತ): <span class="dashed-line">
                                {{ $application->institute->auth->phone ?? '' }} </span></p>
                    </div>
                    <div class="col s12 m5 push-m2 foo-address left">
                        <img src="{{ storage_path('app/public/' . $application->institute->auth->principal_signature ?? '') }}"
                            width="80px" class="mr30" alt="">
                        <img src="{{ storage_path('app/public/' . $application->institute->auth->seal ?? '') }}"
                            width="80px" alt="">
                        <p class="mt10">ಮುಖ್ಯೋಪಾಧ್ಯಾರ/ಪ್ರಾಂಶುಪಾಲರ ಸಹಿ ಹಾಗು ಮೊಹರು</p>
                        <br><br>
                    </div>
                    <div class="clearfix"></div>
                    <div class="col s12 center foo-address">
                        <p>( ವಿ.ಸೂ: ಶಿಕ್ಷಣ ಸಂಸ್ಥೆಯ ಪೂರ್ಣ ವಿಳಾಸವುಳ್ಳ ಮೊಹರು ತಪ್ಪದೆ ಹಾಕುವುದು. )</p>
                    </div>
                </div>
            </div>
        </div>
