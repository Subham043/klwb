...
export default function ActivityLogInfo({
  ...
  const changesMade = useMemo(() => {
    if (
      ...
      !Array.isArray(modal.data.properties.attributes) &&
      modal.data.properties?.old &&
      !Array.isArray(modal.data.properties.old)
    ) {
      return Object.keys(modal.data.properties.attributes).map(
        (key) =>
          `The field named "${key.replace("_", " ")}" changed from ${modal.data?.properties.old[key]} to ${modal.data?.properties.attributes[key]}`,
      );
    }
    if (modal.data && !Array.isArray(modal.data.properties.attributes)) {
      return Object.keys(modal.data.properties.attributes).map(
        (key) =>
          `The field named "${key.replace("_", " ")}" was changed to ${modal.data?.properties.attributes[key]}`,
      );
    }
    return [];
  }, [modal.data]);
            ...
            <Grid fluid>
              ...
              <Row gutter={30}>
                <Col className="pb-1" md={12} sm={24} xs={24}>
                  <DetailInfo
                    title="Name"
                    value={modal.data?.causer?.name || ""}
                  />
                </Col>
                <Col className="pb-1" md={12} sm={24} xs={24}>
                  <DetailInfo
                    title="Email"
                    value={modal.data?.causer?.email || ""}
                  />
                </Col>
              </Row>
              <Row gutter={30}>
                <Col className="pb-1" md={12} sm={24} xs={24}>
                  <DetailInfo
                    title="Phone"
                    value={modal.data?.causer?.phone || ""}
                  />
                </Col>
                <Col className="pb-1" md={12} sm={24} xs={24}>
                  <DetailInfo
                    title="Role"
                    value={modal.data?.causer?.role || ""}
                  />
                </Col>
              </Row>
              <Row gutter={30}>
