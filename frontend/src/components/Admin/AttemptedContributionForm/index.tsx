...
import { useAttemptedContributionQuery } from "../../../hooks/data/attempted_contribution";
...
const schema: yup.ObjectSchema<SchemaType> = yup
  .object({
    male: yup
      .number()
      .typeError("Male must contain numbers only")
      .required("Male is required"),
    female: yup
      .number()
      .typeError("Female must contain numbers only")
      .required("Female is required"),
    price: yup
      .number()
      .typeError("Price must contain numbers only")
      .required("Price is required"),
    interest: yup
      .number()
      .typeError("Interest must contain numbers only")
      .required("Interest is required"),
  })
  .required();

export default function AttemptedContributionForm({
  drawer,
  drawerHandler,
  refetch,
}: {
  ...
  const onSubmit = handleSubmit(async () => {
    ...
    try {
      await axios.post(
        drawer.type === "Edit"
          ? api_routes.admin.attempted_contribution.update(drawer.id)
          : api_routes.admin.attempted_contribution.update(0),
        getValues()
      );
