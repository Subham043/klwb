...
export default function Scholarship({ id }: Props) {
  ...
  const {
    ...
    error,
    refetch: refetchData,
  } = useRegisteredInstitutesScholarshipQuery(Number(id) || 0);
  const { excelLoading, exportExcel } = useExcelExport();

  const excelHandler = async () => {
    await exportExcel(
      api_routes.admin.registered_institute.scholarship.excel(Number(id)),
      "registered_institute_scholarships.xlsx"
    );
  };
