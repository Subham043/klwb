...
const schema: yup.ObjectSchema<SchemaType> = yup
  ...
  phone: yup.number().typeError("Phone must contain numbers only").positive().required("Phone is required"),
		is_blocked: yup.number().typeError("Active/Blocked must contain numbers only").min(0).max(1).required("Active/Blocked is required"),
	})
	.required();

const IndustryAuthUpdate = ({ modal, setModal, data, refetch, error, loading:dataLoading }: Props) => {
		...
		<Modal size={'md'} open={modal} onClose={() => setModal(false)} className='info-modal'>
			<Form onSubmit={()=>onSubmit()} style={{ width: '100%' }}>
				<ErrorBoundaryLayout loading={dataLoading} error={error} refetch={refetch}>
								<>
												<Panel header="Industry Credential Update" className='info-modal-panel' bordered>
