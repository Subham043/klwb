...
import { useInstituteEmployeeQuery } from '../../../hooks/data/institute_employee';
...
export default function InstituteEmployeeForm({drawer, drawerHandler, refetch}:{drawer: DrawerProps; drawerHandler: (value:DrawerProps)=>void; refetch: ()=>void}) {
    const [loading, setLoading] = useState<boolean>(false);
    const {toastError, toastSuccess} = useToast();
    const {data, isFetching, isLoading, isRefetching, error, refetch:refetchData } = useInstituteEmployeeQuery(drawer.type === "Edit" ? drawer.id : 0, (drawer.type === "Edit" && drawer.status && drawer.id>0));
    ...
    const onSubmit = handleSubmit(async () => {
        ...
        try {
            ...
            await axios.post(drawer.type === "Edit" ? api_routes.institute.employee.update(drawer.id) : api_routes.institute.employee.create, {...rest, is_blocked: is_blocked===1 ? 0: 1});
