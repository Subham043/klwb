...
export default function ResetWithPhone(props: {token: string; type: string}) {
    ...
    const onSubmit = handleSubmit(async () => {
        setLoading(true);
        try {
            await axios.post(api_routes.auth.reset_password.phone + `/${props.token}`, getValues());
    ...
    return (
        <Form onSubmit={()=>onSubmit()} style={{ width: '100%' }}>
            <TextInput name="phone" label="Phone" helpText="Please Enter your registered phone" focus={true} control={control} error={errors.phone?.message} />
            <TextInput name="otp" label="OTP" helpText="Please Enter the OTP sent to your mobile number and email" control={control} error={errors.otp?.message} />
