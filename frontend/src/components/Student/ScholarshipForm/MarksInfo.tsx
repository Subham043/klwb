...
export default function MarksInfo({ control, errors, watch }: PropType) {
		...
		<div className="mb-1">
			<Panel header={
				...
				<Stack alignItems="flex-start" direction={isMobile ? 'column' : 'row'} spacing={10} className='info-modal-stack mb-1'>
					...
					{marks_card_type === '0' && <FileInput name="prv_markcard2" accept='image/png, image/jpeg, image/jpg' label="Attach Your Marks Card Copy" helpText=" Only JPG, JPEG, PNG images are allowed (It should be less than 515kb)" control={control} error={errors.prv_markcard2?.message} />}
					<div></div>
				</Stack>
			</Panel>
		</div>
