...
export default function ScholarshipForm({ data, type="apply" }: Props) {
	...
	const {
		...
		values: {
			...
			course_id: data ? data.mark.course_id : undefined,
			course: data && data.mark.course ? { value: data.mark.course.id, label: data.mark.course.name } : { value: 0, label: "" },
			class_id: data ? data.mark.class_id : undefined,
			class: data && data.mark.class ? { value: data.mark.class.id, label: data.mark.class.name } : { value: 0, label: "" },
