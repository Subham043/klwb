...
export default function ScholarshipForm({ data, type="apply" }: Props) {
	...
	const {
		...
		values: {
			...
			graduation: data && data.mark.graduation ? { value: data.mark.graduation.id, label: data.mark.graduation.name } : { value: 0, label: '' },
			course_id: data ? (data.mark.course_id ? data.mark.course_id : undefined) : undefined,
			course: data && data.mark.course ? { value: data.mark.course.id ? data.mark.course.id : undefined, label: data.mark.course.name ? data.mark.course.name : undefined } : { value: undefined, label: undefined },
			class_id: data ? (data.mark.class_id ? data.mark.class_id : undefined) : undefined,
			class: data && data.mark.class ? { value: data.mark.class.id ? data.mark.class.id : undefined, label: data.mark.class.name ? data.mark.class.name : undefined } : { value: undefined, label: undefined },
