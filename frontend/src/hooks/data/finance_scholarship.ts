    ...
    queryFn: async () => {
      const response = await axios.get<PaginationType<StudentApplicationType>>(
        api_routes.finance.scholarship.list +
          `?page=${page}&total=${limit}&filter[search]=${search}&filter[has_city]=${searchParams.get("city_id") || ""}&filter[has_taluq]=${searchParams.get("taluq_id") || ""}&filter[has_graduation]=${searchParams.get("graduation_id") || ""}&filter[has_course]=${searchParams.get("course_id") || ""}&filter[has_class]=${searchParams.get("class_id") || ""}&filter[application_year]=${searchParams.get("year") || ""}&filter[payment_status]=${searchParams.get("payment_status") || ""}&filter[has_gender]=${searchParams.get("gender") || ""}&filter[has_category]=${searchParams.get("category") || ""}`
      );
