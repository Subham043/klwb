...
const City:FC = () => {
    const {data, isLoading, isFetching, isRefetching, refetch} = useCitiesQuery();
    ...
    const excelHandler = async () => {
        await exportExcel(api_routes.admin.city.excel, 'city.xlsx');
    }

    const onDeleteHandler = async (id:number) => {
        await deleteHandler(api_routes.admin.city.delete(id));
        refetch();
    }

    return <PaginatedTableLayout title="Cities" addHandler={() => setOpenDrawer({status:true, type:'Create'})} total={(data?.meta.total ?? 0)} excelHandler={excelHandler} excelLoading={excelLoading}>
        <Table
