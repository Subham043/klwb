...
const FinanceScholarshipListPage: FC = () => {
  const { data, isLoading, isFetching, isRefetching, refetch, error } =
    useFinanceScholarshipListQuery();
  ...
  const onMultipleApproveHandler = async () => {
    setLoading(true);
    try {
      await axios.post(api_routes.finance.scholarship.approve_multiple, {
        id: checkedValue,
      });
    ...
    <>
      <PaginatedTableLayout title="Scholarship List">
        <PaginatedTableLayout.Header
          ...
          addBtn={false}
          excelLink={api_routes.finance.scholarship.excel}
          excelName="applications.xlsx"
        >
          {(searchParams.get("payment_status") === "pending" ||
            searchParams.get("payment_status") === "rejected") &&
        ...
        >
          ...
          >
            ...
            <Table.Column width={170} verticalAlign="middle">
              ...
              <Table.Cell style={{ padding: "6px" }}>
                {(rowData) => (
                  <StatusBadge
                    ...
                    current_application_state={4}
                    current_application_state_name="ADMIN"
                  />
                )}
              </Table.Cell>
            </Table.Column>
            ...
            <Table.Column width={130} fixed="right" verticalAlign="middle">
              ...
              <Table.Cell style={{ padding: "6px" }}>
                {(rowData) => (
                  <ButtonToolbar>
                    <ViewLink
                      to={page_routes.finance.scholarship.view(rowData.id)}
                    />
                    {(rowData.pay_status === 0 || rowData.pay_status === 2) && (
                      <Whisper
                        placement="bottomEnd"
                    ...
                    {(rowData.pay_status === 0 || rowData.pay_status === 1) && (
                      <Whisper
                        placement="bottomEnd"
                        controlId="control-id-click"
                        trigger="hover"
                      ...
                      >
                        <IconButton
                          ...
                          icon={<CloseIcon />}
                          onClick={() =>
                            setModal({ open: true, id: rowData.id })
                          }
                        />
                      </Whisper>
                    )}
      ...
      <FinanceScholarshipRejectForm
        modal={modal.open}
        setModal={(value) => setModal({ ...modal, open: value })}
        id={modal.id}
        refetch={refetch}
      />
    </>
