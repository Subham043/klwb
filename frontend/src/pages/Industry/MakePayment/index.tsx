...
import {
  ...
  Contract_Labour_Act,
  Factory_Act,
  Shops_and_Commercial_Act,
  Society_Registration_Act,
} from "../../../utils/constants/company_act";

const isAfter31stDec = moment().isAfter(moment().year() + "-12-31", "day");

const diff = moment().year() - (isAfter31stDec ? 2016 : 2017);

const years = Array.from({ length: diff }, (_, i) => ({
  label: (moment().year() - (isAfter31stDec ? i : i + 1)).toString(),
  value: (moment().year() - (isAfter31stDec ? i : i + 1)).toString(),
}));
...
export default function MakePaymentPage() {
  ...
  const {
    ...
    values: {
      ...paymentFormInitialValues,
      act:
        account_info && account_info.industry.act
          ? account_info.industry.act.toString()
          : "",
      category: account_info ? account_info.industry.category || "" : "",
    },
  });

  const { data, isFetching, isLoading, isRefetching, refetch } =
    useIndustryPaymentPaidYearQuery(true);
  ...
  const charge = useMemo(
    () => (selectedYear && selectedYear < 2025 ? 60 : 150),
    [selectedYear]
  );

  const amount = useMemo(
    () => (+maleCount + +femaleCount) * charge,
    [maleCount, femaleCount, charge]
  );
  ...
  const interest = useMemo(() => {
    ...
    const month_diff = moment(curYear - 1 + "-" + curMonth + "-" + curDay).diff(
    ...
    if (diff == 0 || diff == 1) {
      if (curMonth == 1 && curDay >= 1 && curDay <= 15) {
        ...
        (curMonth == 1 && curDay >= 16) ||
        (curMonth <= 3 && curDay <= 31)
      ) {
  ...
  const onSubmit = handleSubmit(async () => {
    ...
    if (+maleCount + +femaleCount < requiredNoOfEmployees) {
      toastError(
        "Minimum of " +
          ...
          " employees in total is required to be eligible for contribution."
      );
      return;
    }
    setLoading(true);
    try {
      ...
      const resp = await axios.post<{ data: string }>(
        api_routes.industry.payment.make_payment,
        formData
      );
  ...
  return (
    <div className="data-table-container">
      ...
      >
        <Divider />
        <ErrorBoundaryLayout
          loading={isAccountLoading || isAccountFetching || isAccountRefetching}
          error={accountError}
          refetch={accountRefetch}
        >
              ...
              <Form.Group>
                      ...
                      <Row gutter={30}>
                        ...
                        <Col className="pb-1" md={8} sm={24} xs={24}>
                          <SelectInput
                            ...
                            data={years
                              .filter(
                                (year) => !data?.includes(Number(year.value))
                              )
                              .map((year) => ({
                                label: year.label,
                                value: year.value,
                            ...
                            <FileInput
                              ...
                              helpText={
                                <span>
                                  ...
                                  <a
                                    href="/employee_excel_format.xlsx"
                                    download={true}
                                  >
                                    click here
                                  </a>{" "}
                        ...
                        <Col className="pb-1 pt-1" sm={24} xs={24}>
                          <div className={classes.table_container}>
                            <table className={classes.table} border={1}>
                              <thead>
                                {requiredNoOfEmployees !== undefined && (
                                  <tr className={classes.bg_indigo}>
                                    <th colSpan={5}>
                                      Minimum of {requiredNoOfEmployees}{" "}
                                      employees in total is required to be
                            ...
                            <Controller
                              ...
                              render={({ field }) => (
                                <>
                                  ...
                                  >
                                    I understand that my transaction does not
                                    qualify for any chargeback claims
                                  </Checkbox>
                                  <Form.ErrorMessage
                                    show={!!errors.term1?.message}
                            ...
                            <Controller
                              ...
                              render={({ field }) => (
                                <>
                                  ...
                                  >
                                    I Accept that any chargebackis not automatic
                                    and for any chargeback claim the concerned
                                    government department shall be approached.
                                  </Checkbox>
                                  <Form.ErrorMessage
                                    show={!!errors.term1?.message}
                                    placement="bottomStart"
                ...
                >
                  ...
                  <Button
                    ...
                    size="md"
                    type="button"
                    onClick={() => reset(paymentFormInitialValues)}
                  >
                    Reset
                  </Button>
                </ButtonToolbar>
