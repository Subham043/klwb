			...
			<ErrorBoundaryLayout loading={isRefetching} error={error} refetch={refetch}>
				<div style={{ width: "100%", position: "relative" }}>
					{(isFetching || isLoading || isRefetching) ? <CheckingEligibility />
						...
						: 
						((data && data.is_eligible) ? <ScholarshipForm data={data && data.application ? data.application : null} />  : <ApplicationApplied message={data?.message} date={data?.application?.date} />))}
				</div>
			</ErrorBoundaryLayout>
		</Panel>
	</div>
